  ...
  return (
    <div className="detail-backdrop" onClick={() => { if (!deleting) onClose() }}>
      <aside className="detail-drawer" onClick={(event) => event.stopPropagation()}>
        <header><div><small>提交详情</small><h2>{detail.submissionId}</h2><p>{new Date(detail.submittedAt).toLocaleString('zh-CN')}{(detail.revisionCount ?? 0) > 0 ? ` · 已修改 ${detail.revisionCount} 次，当前第 ${detail.version ?? 1} 版` : ` · 第 ${detail.version ?? 1} 版`}</p></div><button className="icon-control" title="关闭" disabled={deleting} onClick={onClose}><X size={18} /></button></header>
